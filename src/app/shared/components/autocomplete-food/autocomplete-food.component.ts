...
export class AutocompleteFoodComponent implements OnInit, OnDestroy {
	...
	private handleArrowDown() {
		...
		if (
			activeElement &&
			activeElement.classList.contains("autocomplete__input")
		) {
	...
	private handleArrowUp() {
		...
		if (
			activeElement &&
			activeElement.classList.contains("autocomplete__option")
		) {
			const index = this.getCurrentOptionIndex();
			if (index === 0) {
				const element = document.getElementsByClassName(
	...
	private closeAutoComplete(target: EventTarget | null) {
		...
		if (
			target &&
			newIngredientEref &&
			!newIngredientEref.nativeElement.contains(target)
		) {
			this.showAutoComplete = false;
			this.foodMatch = [];
		}
